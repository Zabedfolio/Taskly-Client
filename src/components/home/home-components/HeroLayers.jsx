...
// Each layer: top-face diamond points, extrusion depth, glow colours, label
const LAYERS = [
  {
    id: "post",
    label: "POST A TASK",
    // diamond centre-Y for top face
    cy: 178,
    ...
    left: { x: 220, y: 178 },
    depth: 32,
    // radial gradient stops for top face glow
    glow: {
      id: "g0",
      stops: [
        { o: "0%",   c: "#ffffff", a: 0.58 },
...
/** Subtle ISO grid in the background */
function IsoGrid() {
  const stroke = "rgba(180,200,255,0.075)";
  const sw = "0.55";

  // Generate diagonal lines covering the whole viewport
  const rLines = [];
  const lLines = [];
  const step = 112; // horizontal spacing between parallels

  for (let i = -2; i <= 8; i++) {
    const x0 = i * step;
    // right-leaning (\) — slope 0.5
    rLines.push(
      <line key={`r${i}`} x1={x0} y1={0} x2={x0 + H * 2} y2={H} stroke={stroke} strokeWidth={sw} />
    );
    // left-leaning (/) — slope -0.5
    lLines.push(
...
function IsoLayer({ cfg }) {
  ...
  // Label box — to the left of the left vertex
  ...
  return (
    ...
    >
      <defs>
        ...
        {/* Clip for surface grid (bottom layer only) */}
        <clipPath id={clipId}>
          <path d={topPath} />
        </clipPath>
      </defs>

      {/* ── Ambient glow behind layer ── */}
      <ellipse
        cx={(top.x + bot.x) / 2}
        cy={cfg.cy + 12}
        rx={180} ry={100}
      ...
      {/* ── Bottom-layer surface grid (hire layer only) ── */}
      {cfg.id === "hire" && (
        <path
          d={topPath}
          fill="url(#surfgrid)"
      ...
      {/* ── Top face — dark base (gives glass depth) ── */}
      <path
        d={topPath}
        fill="rgba(140,45,0,0.28)"
      />

      {/* ── Top face — glassy radial glow ── */}
      <path
        d={topPath}
        fill={`url(#${glowId})`}
      />

      {/* ── Rim highlight: top two edges (brighter) ── */}
      <path
        d={topRimPath}
        fill="none"
        stroke={cfg.rimTop}
  ...
  return (
    <div className="relative flex items-center justify-center select-none">
      ...
      >
        ...
        {/* Render bottom-up so top layer paints last */}
