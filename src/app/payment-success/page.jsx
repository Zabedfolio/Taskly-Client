...
function ErrorState({ message }) {
    return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, padding: '32px 20px', textAlign: 'center' }}>
            <div style={{
...
function PaymentSuccessContent() {
    ...
    useEffect(() => {
        if (!sessionId || !proposalId) {
            setError('Missing payment parameters. This link may be invalid.');
            setLoading(false);
            return;
        }
        ...
        async function verify() {
            try {
                ...
                setError(err.message || 'Failed to verify your payment with the backend.');
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <>
            ...
            <div style={{
                ...
                {/* Outer wrapper to layer the background glow properly */}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 22, marginBottom: 44 }}>
                            ...
                            <div style={{ animation: 'fadeUp 0.5s 0.85s both ease' }}>
                                ...
                                <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', maxWidth: 360, margin: '0 auto', lineHeight: 1.7 }}>
                                    Your Stripe transaction was verified. The task is now{' '}
                                    <strong style={{ color: 'rgba(255,255,255,0.75)', fontWeight: 700 }}>active</strong>{' '}
                                    and the worker has been officially hired.
                                </p>
                            </div>
